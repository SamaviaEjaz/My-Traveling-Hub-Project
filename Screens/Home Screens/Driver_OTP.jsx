  ...
  const otpValidate = async () => {
    ...
    try {
      ...
      const res = await axios.post(
        `${BASE_URL}/api/drivers/verify-otp`,
        { phone, otp: enteredOtp }
      );

      console.log("OTP verification response:", res.data);

      // ✅ Fix — OK button ke saath Pending screen pe navigate karo
      Alert.alert(
        "✅ Registration Submitted",
        "Your registration has been submitted for approval.\n\nPlease login within 24 hours to check your approval status.",
        [
          {
            text: "OK",
    ...
    try {
      ...
      const handleOTPVerification = async () => {
        try {
          ...
          const response = await fetch(`${BASE_URL}/api/drivers/verify-otp`, {
            method: 'POST',
            headers: {
